...
void As1::UpdateCamera(const Ogre::FrameEvent& fe){
	float move = 400.0f;
	float rotate = 0.1f;
...
void As1::UpdateVelocity(const Ogre::FrameEvent& fe){
...
void As1::UpdatePosition(const Ogre::FrameEvent& fe){
	position = position + (velocity * fe.timeSinceLastFrame);
}

void As1::CopyPositionToSceneNode(){
